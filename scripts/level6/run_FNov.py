...
CRITERIA_GENERATION_PROMPT_TEMPLATE = load_prompt("level6/FNov_criteria_generation.txt")
SCORING_PROMPT_TEMPLATE = load_prompt("level6/FNov_scoring.txt")
...
class FrameworkNoveltyEvaluator:
    ...
    def calculate_final_score(
        ...
        try:
            ...
            if not scored_items:
                raise KeyError("'framework_novelty' key not found in scores.")
            ...
            if abs(total_weight - 1.0) > 1e-6 and total_weight > 0:
                print(f"Warning: Criteria weights sum to {total_weight}, not 1.0.")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Framework Novelty (FNov) evaluation tool")
    ...
    criteria_path = output_dir / "framework_novelty_criteria.json"
    scoring_path = output_dir / "comparative_scores_novelty.json"
